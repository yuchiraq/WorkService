...
func UsersPage(c *gin.Context) {
    ...
    <meta name="viewport" content="width=device-width, initial-scale=1, viewport-fit=cover"><title>Пользователи</title><link rel="stylesheet" href="/static/css/style.css?v=11"></head><body>
{{SIDEBAR_HTML}}
...
func renderUserForm(c *gin.Context, user models.User, actionURL, title, submitLabel string, adminEditable bool) {
	...
	page := `<!DOCTYPE html><html lang="ru"><head><meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, viewport-fit=cover"><title>{{TITLE}}</title><link rel="stylesheet" href="/static/css/style.css?v=11"></head><body>
{{LAYOUT_START}}
...
<div class="form-group-edit form-group-rate"><label for="phone">Контактный номер</label><input type="tel" id="phone" name="phone" value="{{PHONE}}"></div>`
	...
	page := `<!DOCTYPE html><html lang="ru"><head><meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, viewport-fit=cover"><title>Мой профиль</title><link rel="stylesheet" href="/static/css/style.css?v=11"></head><body>
{{SIDEBAR_HTML}}
